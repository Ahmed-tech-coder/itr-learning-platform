...
const LectureView = () => {
  ...
  // 🔹 Fetch current lecture whenever lectureId changes
  useEffect(() => {
    if (!lectureId) return;
    const fetchLecture = async () => {
      try {
        const res = await fetch(
          `${BASE_API}/Lecture/GetLecture?LectureId=${lectureId}`,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("adminToken")}`,
            },
  ...
  if (!lecture)
    return <div className="text-white p-10">جارِ تحميل المحاضرة...</div>;
  ...
  const handleNavigate = (lec) => {
    if (!lec) return;
    navigate(`/courses/${courseId}/lectures/${lec.id}/lecture-view`);
  };
    ...
    <div className="p-6 pt-24 md:p-10">
      ...
      >
        <Tabs value={activeTab} onValueChange={setActiveTab} className="mb-8">
          ...
          {/* Video Section */}
          <div
            className={`mt-6 flex flex-col items-center space-y-6 ${activeTab === "video" ? "block" : "hidden"
              }`}
          >
            <motion.div
